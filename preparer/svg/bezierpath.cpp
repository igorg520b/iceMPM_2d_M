...
void BezierPath::print() const
{
    ...
    std::cout << "Path " << name << "; "
              << (closed ? "Closed" : "Open") << "; winding "
              << winding << "; nseg " << segments.size() << std::endl;
...
{
    std::cout << "rendering path idx " << pathIdx << std::endl;
...
//        path_indices[index] = pathIdx_;
    ...
    {
        std::cout << "rendering path " << pathIdx << "; segment " << segIdx << " of " << segments.size() << std::endl;
...
// =========================== export as ANSYS python script


bool BezierPath::isLineLike(const BezierSegment& seg, float tol)
{
    // Vector P0->P3
    Eigen::Vector2f p0(seg.x0, seg.y0);
    Eigen::Vector2f p3(seg.x3, seg.y3);
    Eigen::Vector2f d = p3 - p0;

    // Degenerate line?
    if (d.norm() < tol) return false;

    // Check distances of P1 and P2 from line P0->P3
    Eigen::Vector2f p1(seg.x1, seg.y1);
    Eigen::Vector2f p2(seg.x2, seg.y2);
    float dist1 = std::abs((d.x()*(p0.y()-p1.y()) - d.y()*(p0.x()-p1.x())) / d.norm());
    float dist2 = std::abs((d.x()*(p0.y()-p2.y()) - d.y()*(p0.x()-p2.x())) / d.norm());

    return (dist1 < tol && dist2 < tol);
}


void BezierPath::exportToAnsysDiscovery(const std::string& filename, int samplesPerSegment) const
{
    ...
    ofs << "# Python Script, API Version = V242\n\n";
    ofs << "plane = Plane.PlaneXY\n";
    ofs << "result = ViewHelper.SetSketchPlane(plane)\n";

    for (const auto& seg : segments)
    {
        if (isLineLike(seg)) {
            // Emit straight line
            ofs << "start = Point2D.Create(IN(" << seg.x0 << "), IN(" << seg.y0 << "))\n";
            ofs << "end = Point2D.Create(IN(" << seg.x3 << "), IN(" << seg.y3 << "))\n";
            ofs << "result = SketchLine.Create(start, end)\n";
            ofs << "# EndBlock\n";
        }
        else {
            // Emit sampled NURBS spline
            ofs << "points = List[Point2D]()\n";
            for (int j = 0; j <= samplesPerSegment; ++j) {
                float t = static_cast<float>(j) / samplesPerSegment;
                float x, y;
                evaluateBezier(seg, t, x, y);
                ofs << std::fixed << std::setprecision(6);
                ofs << "points.Add(Point2D.Create(IN(" << x << "), IN(" << y << ")))\n";
            }
            ofs << "result = SketchNurbs.CreateFrom2DPoints(False, points)\n";
            ofs << "# EndBlock\n";
        }
    }

    // If closed, add final connecting line/spline
    if (closed && !segments.empty()) {
        const auto& first = segments.front();
        const auto& last = segments.back();
        ofs << "start = Point2D.Create(IN(" << last.x3 << "), IN(" << last.y3 << "))\n";
        ofs << "end = Point2D.Create(IN(" << first.x0 << "), IN(" << first.y0 << "))\n";
        ofs << "result = SketchLine.Create(start, end)\n";
        ofs << "# EndBlock\n";
    }
}
