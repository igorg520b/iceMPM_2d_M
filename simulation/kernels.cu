...
__global__ void partition_kernel_update_nodes(const PartitionParams pparams, const t_PointReal simulation_time)
{
    ...
    const GridVector2r gnpos = gi.cast<t_GridReal>()*gprms.cellsize;    // position of the grid node in the whole grid
...
//    const size_t &gridX_offset = pparams.pts_gridX_offset;
    ...
    const uint32_t cell = *reinterpret_cast<const uint32_t*>(&bpts[pt_idx + pitch_pts*SimParams::integer_cell_idx]);
...
//    const size_t &gridX = pparams.pts_partition_gridX;
    ...
    auto transferPoint = [&](int bufferIndex, unsigned int* counter) {
        ...
        {
            t_PointReal *ptb = pparams.point_transfer_buffer[bufferIndex];
            for(int i=0;i<SimParams::nPtsArrays;i++)
                ptb[i + flyIdx*SimParams::nPtsArrays] = bpts[pt_idx + pitch_pts*i];
                                                ...
                                                const unsigned bufferIdx)
{
    ...
    t_PointReal* const &bpts = pparams.buffer_pts;
    const size_t &pitch_pts = pparams.pitch_pts;
    t_PointReal* const transfer_buffer = pparams.point_transfer_buffer[bufferIdx];
    ...
    for(int i=0;i<SimParams::nPtsArrays;i++)
    {
        bpts[idx_in_soa + i*pitch_pts] = transfer_buffer[i + SimParams::nPtsArrays*pt_idx];
    }
}
